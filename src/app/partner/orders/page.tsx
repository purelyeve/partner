...
export default async function PartnerOrdersPage({
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-pe-brown">
            Paid customer invoices awaiting fulfillment, shipped, or refunded.
          </p>
        </div>
        <div className="flex flex-wrap gap-2 text-sm">
          <Link href="/partner/orders?filter=pending" className="nav-link">
            Pending
          </Link>
          <Link href="/partner/orders?filter=shipped" className="nav-link">
            Shipped
          </Link>
          <Link href="/partner/orders?filter=refunded" className="nav-link">
            Refunded
          </Link>
          <Link href="/partner/orders" className="nav-link">
            All
          </Link>
        </div>
      </div>
      ...
      {!orders?.length ? (
        <Card>
          <p className="text-sm text-pe-brown">No orders in this view yet.</p>
        </Card>
      ) : (
                ...
                return (
                  <tr key={o.id} className="border-t border-pe-beige">
                    ...
                    <td className="p-3 text-right">
                      <Link href={`/partner/orders/${o.id}`}>Open</Link>
                    </td>
                  </tr>
                )
              })}
